...
if [ $# -eq 0 ]; then
	...
	echo " -b | --branch <branch>         # Clone specified git branch"
...
if [ -d $1 ]; then
	...
	case "${1##*.}" in
		"git")
			...
			git clone $BRANCH --recursive $1 $MODULE_DIR
			;;
		"zip")
...
PKG_OSS_URL="https://github.com/natefoo/pkg-oss"

git clone $PKG_OSS_URL

if [ "$BUILD_PLATFORM" = "OSS" ]; then
	if [ "$OSS_VER" != "" ]; then
		( cd pkg-oss && git checkout `git tag -l | grep "^$OSS_VER" | tail -1 | awk '{print $1}'` )
	fi
else
